...
/* s21_strcat: append src to dest including terminating '\0'.
   Behavior: if dest == NULL -> return NULL; if src == NULL -> return dest unchanged. */
...
char *s21_strchr(const char *s, int c) {
	...
	/* check terminating '\0' */
	if (uc == (unsigned char)'\0') return (char *)s;
	return NULL;
}
...
static int s21_is_delim(unsigned char ch, const char *delim) {
	...
	const char *d = delim;
	while (*d) {
		if ((unsigned char)*d == ch) return 1;
		++d;
	}
	return 0;
}
...
/* s21_strtok: tokenization similar to standard strtok.
   ...
   - Returns pointer to token (modifies input buffer by inserting '\\0') or NULL if no more tokens.
   - Safe with NULL pointers: if delim == NULL -> NULL; if both str and saved state NULL -> NULL. */
char *s21_strtok(char *str, const char *delim) {
	...
	while (*p && s21_is_delim((unsigned char)*p, delim)) ++p;
	if (*p == '\0') {
		saveptr = NULL;
		return NULL;
	}
