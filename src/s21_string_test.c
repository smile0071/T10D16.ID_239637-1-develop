...
void s21_strcmp_test(void) {
	...
	for (size_t i = 0; i < num; ++i) {
		...
		if (s1 == NULL) printf("Input s1: NULL\n"); else printf("Input s1: \"%s\"\n", s1);
		if (s2 == NULL) printf("Input s2: NULL\n"); else printf("Input s2: \"%s\"\n", s2);
...
void s21_strcat_test(void) {
	const struct { const char *d; const char *s; const char *expect; } tests[] = {
	...
	for (size_t i = 0; i < num; ++i) {
		...
		if (init_dest) {
			...
			size_t j = 0;
			while (init_dest[j]) { dest[j] = init_dest[j]; ++j; }
			dest[j] = '\0';
		} else {
			dest[0] = '\0';
		}
		/* print inputs */
		printf("Input dest: \"%s\"\n", dest);
		if (src == NULL) printf("Input src: NULL\n"); else printf("Input src: \"%s\"\n", src);
		/* call */
...
void s21_strstr_test(void) {
	const struct { const char *h; const char *n; const char *expect; } tests[] = {
	...
	for (size_t i = 0; i < num; ++i) {
		...
		const char *expected = tests[i].expect;
		if (h == NULL) printf("Input haystack: NULL\n"); else printf("Input haystack: \"%s\"\n", h);
		if (n == NULL) printf("Input needle: NULL\n"); else printf("Input needle: \"%s\"\n", n);
...
void s21_strtok_test(void) {
	...
	/* Test 1: normal comma-separated */
	{
		...
		printf("Input: \"%s\"\nDelim: \"%s\"\n", buf, delim);
		int ok = 1;
		char *t = s21_strtok(buf, (char *)delim); /* cast only for readability; s21_strtok signature expects char* */
		size_t i = 0;
