...
int main(int argc, char **argv) {
	...
	while (cur < wcount) {
		...
		int count = 0;
		while (cur < wcount) {
			int wl = 0; while (words[cur][wl]) ++wl;
			int needed = (count == 0) ? wl : (len + 1 + wl);
			if (needed <= width) {
				if (count == 0) len = wl; else len = len + 1 + wl;
				++cur; ++count;
			} else break;
		}
	...
	/* output lines: build single buffer without trailing newline and print it */
	{
		...
		if (total == 0) {
			...
			if (out) {
				...
				for (int i = 0; i < lcount; ++i) {
					char *s = lines[i];
					int k = 0; while (s[k]) ++k;
					for (int j = 0; j < k; ++j) out[pos++] = s[j];
					if (i + 1 < lcount) out[pos++] = '\n';
				}
				out[pos] = '\0';
				/* single write without trailing newline */
				fputs(out, stdout);
				free(out);
			}
		}
